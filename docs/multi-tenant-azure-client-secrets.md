...
- Add a federated credential trust on each new registration, matching Yale's existing OIDC-federation shape (`api://AzureADTokenExchange` audience, subject bound to the relevant cluster's Yale **GCP service account**, not a Kubernetes-issued token — Yale authenticates via GCE/GKE metadata-server OIDC token exchange, with `https://accounts.google.com` as the issuer; see the Appendix for exactly how many federated credentials this means per tenant) — same no-stored-secret pattern Yale already uses today.
...
Per Y-C, the federated credential's subject is the relevant cluster's Yale **GCP service account (GSA)**, not a Kubernetes-issued token: [`client.go`](../internal/yale/client/client.go)'s `buildAzureGraphClient()` gets its OIDC token via `google.FindDefaultCredentials` + `idtoken.NewTokenSource` (the GCE/GKE metadata server), authenticating as a GCP service account with `https://accounts.google.com` as the OIDC issuer. The federated credentials configured below all trust Google service account identities.
