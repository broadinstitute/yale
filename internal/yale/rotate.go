...
func (m *Yale) CreateSecret(gsk apiv1b1.GCPSaKey) (*corev1.Secret, error) {
	...
	createdSecret, err := m.k8s.CoreV1().Secrets(gsk.Namespace).Create(context.TODO(), newSecret, metav1.CreateOptions{})
...
func (m *Yale) UpdateKey(gskSpec apiv1b1.GCPSaKeySpec, namespace string) (*corev1.Secret, error) {
	...
	updatedSecret, err := m.UpdateSecret(K8Secret)
	if err != nil {
		return nil, err
	}
...
func (m *Yale) UpdateSecret(k8Secret *corev1.Secret) (*corev1.Secret, error) {
	updatedSecret, err := m.k8s.CoreV1().Secrets(k8Secret.Namespace).Update(context.TODO(), k8Secret, metav1.UpdateOptions{})
	if err != nil {
