...
type Clients struct {
	...
	azure         map[string]*msgraph.ApplicationsClient
...
// GetAzure returns a handle to the Microsoft Graph clients generated by the builder, keyed by Azure AD tenant ID
func (c *Clients) GetAzure() map[string]*msgraph.ApplicationsClient {
	return c.azure
}
...
func Build(local bool, kubeconfig string, disableVaultReplication bool) (*Clients, error) {
	...
	azure, err := buildAzureGraphClients(local)
	if err != nil {
		return nil, fmt.Errorf("error building Azure Graph clients: %v", err)
	}
...
// azureAppRegistrationCredential identifies the Azure AD app registration Yale should
// authenticate as (via federated OIDC token exchange) to manage client secrets in one tenant.
type azureAppRegistrationCredential struct {
	TenantID string `json:"tenantId"`
	ClientID string `json:"clientId"`
}

const yaleAzureCredentialsEnvVar = "YALE_APP_REGISTRATION_CREDENTIALS"

// Superseded by yaleAzureCredentialsEnvVar's JSON list, which supports more than one tenant.
// Kept as a fallback so this binary can still start against a not-yet-migrated Helm release
// (the two rollouts are in different repos and can land out of order) -- remove once all
// clusters are confirmed migrated to yaleAzureCredentialsEnvVar. See
// docs/multi-tenant-azure-client-secrets.md.
const (
	yaleLegacyClientIDEnvVar string = "YALE_APP_REGISTRATION_CLIENT_ID"
	yaleLegacyTenantIDEnvVar string = "YALE_APP_REGISTRATION_TENANT_ID"
)

// getYaleAppRegistrationCredentials reads the list of {tenantId, clientId} credentials Yale
// should build a Microsoft Graph client for, one per Azure AD tenant it manages secrets in.
func getYaleAppRegistrationCredentials() ([]azureAppRegistrationCredential, error) {
	if raw, ok := os.LookupEnv(yaleAzureCredentialsEnvVar); ok {
		var credentials []azureAppRegistrationCredential
		if err := json.Unmarshal([]byte(raw), &credentials); err != nil {
			return nil, fmt.Errorf("error parsing %s as a JSON list of {tenantId, clientId} credentials: %v", yaleAzureCredentialsEnvVar, err)
		}
		if len(credentials) == 0 {
			return nil, fmt.Errorf("%s must contain at least one tenantId/clientId credential", yaleAzureCredentialsEnvVar)
		}
		return credentials, nil
	}

	tenantID, tenantOk := os.LookupEnv(yaleLegacyTenantIDEnvVar)
	if !tenantOk {
		return nil, fmt.Errorf("%s not set (legacy fallback %s also not set)", yaleAzureCredentialsEnvVar, yaleLegacyTenantIDEnvVar)
	}
	clientID, clientOk := os.LookupEnv(yaleLegacyClientIDEnvVar)
	if !clientOk {
		return nil, fmt.Errorf("%s not set", yaleLegacyClientIDEnvVar)
	}
	return []azureAppRegistrationCredential{{TenantID: tenantID, ClientID: clientID}}, nil
}

// buildAzureGraphClients builds one Microsoft Graph client per configured Azure AD tenant,
// keyed by tenant ID, so azurekeyops can route each AzureClientSecret CRD to the client for
// the tenant its app registration actually lives in.
func buildAzureGraphClients(local bool) (map[string]*msgraph.ApplicationsClient, error) {
	credentials, err := getYaleAppRegistrationCredentials()
	if err != nil {
		return nil, fmt.Errorf("error getting Yale app registration credentials: %v", err)
	}
	...
	for _, cred := range credentials {
		...
		if err != nil {
			return nil, fmt.Errorf("error building Azure Graph client for tenant %s: %v", cred.TenantID, err)
		}
		clients[cred.TenantID] = client
	}
